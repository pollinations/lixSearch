...
\textbf{Redis key structure.} Each session uses two Redis keys: (1)~a list key tracking turn IDs in insertion order, and (2)~per-turn keys storing individual message JSON, each with independent TTL. Keys are namespaced by a configurable prefix and the session ID.
