...
\title{Scaling LLM-Powered Web Search on CPU:\\A Three-Layer Caching Architecture with 89\%\\Hit Rate at 0.1\,ms Latency}
...
With the problems identified, the architecture of \texttt{lix\_cache} took shape around a simple principle: each problem gets its own caching layer, each layer gets its own Redis database, and a single coordinator fa\c{c}ade ties them together. Figure~\ref{fig:architecture} illustrates the complete data flow when a user message arrives.
...
The original motivation for building \texttt{lixSearch} was cost. Figure~\ref{fig:cost} shows the per-query cost comparison between OpenAI's web search API and our system with and without caching. With the semantic cache achieving an 89.3\% hit rate, the effective cost per query drops further-cache hits cost essentially nothing, as no LLM call or search is needed.
...
\begin{axis}[
    ...
    symbolic x coords={SearchGPT API, lixSearch (no cache), lixSearch (cached)},
    xtick=data,
...
\addplot[fill=red!40] coordinates {
    (SearchGPT API, 0.22) [\$0.220]
    (lixSearch (no cache), 0.003) [\$0.003]
    (lixSearch (cached), 0.0003) [\$0.0003]
};
\end{axis}
\end{tikzpicture}
\caption{Per-query cost comparison (log scale). SearchGPT API includes the hidden context token charge (${\sim}$8k tokens per search). lixSearch uses open models with Playwright-based search agents. The cached cost reflects the 89.3\% hit rate, where cache hits incur zero LLM cost.}
\label{fig:cost}
\end{figure}
...
\begin{axis}[
    ...
    symbolic y coords={
        Redis GET,
        Redis pipeline (10 cmds),
        Huffman decode 6.4KB,
        Huffman encode 6.4KB,
        Disk load (6 turns),
        Disk load (133 turns)
    },
    ytick=data,
...
(0.107,{Redis GET}) [0.1ms]
(0.351,{Redis pipeline (10 cmds)}) [0.4ms]
(8.0,{Huffman decode 6.4KB}) [8.0ms]
(8.9,{Huffman encode 6.4KB}) [8.9ms]
(3.0,{Disk load (6 turns)}) [3.0ms]
(106.6,{Disk load (133 turns)}) [107ms]
};
\end{axis}
\end{tikzpicture}
\caption{Operation latencies from production containers (log scale). Redis reads are two orders of magnitude faster than disk, confirming the hot-window design. Huffman codec throughput (${\sim}$700--800\,KB/s in pure Python) is sufficient for typical conversation payloads.}
\label{fig:latency}
\end{figure}
