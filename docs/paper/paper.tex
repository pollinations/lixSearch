...
% ============================================================
\begin{abstract}
The past two years have seen a proliferation of AI-powered search products-OpenAI's SearchGPT (now integrated into ChatGPT), Google's AI Overviews, and Perplexity's real-time answer engine-each offering LLM-synthesized answers grounded in live web results. For end users, these are transformative. For developers building on top of them, the economics are punishing: OpenAI charges \$0.22 per search query via API, Google's Gemini with grounding starts at \$35/1K queries, and Perplexity's Sonar API runs \$5/1K requests for the base tier~\cite{perplexity_pricing}. We set out to build an open-source alternative that could search the web with LLMs at a fraction of the cost, using automated browser agents and open models-running entirely on CPU, without GPU acceleration. The system worked, but as users grew, so did the problems. Sessions lost context between turns. Semantically identical queries triggered redundant LLM calls. The same URLs were embedded over and over across sessions. On a resource-constrained 8-vCPU server, every wasted computation mattered.
...
The API pricing across providers is steep. OpenAI charges \$10--30 per thousand search calls as a base fee, plus ${\sim}$8{,}000 tokens of injected web context billed at input rates-a single query can cost \$0.22~\cite{openai_search_pricing, openai_search_billing}. Google's Gemini with grounding starts at \$35 per 1{,}000 grounded queries~\cite{google_grounding_pricing}. Perplexity's Sonar API charges \$5 per 1{,}000 requests for the base model, scaling to \$200/1K for the ``deep research'' tier~\cite{perplexity_pricing}. Developers on OpenAI's community forums reported bills 2--3$\times$ higher than expected, with some paying over \$2 for just 21 searches~\cite{openai_search_billing}. For a search assistant handling thousands of queries per day, this was a non-starter.
...
The system also introduced a \textbf{two-tier hybrid storage model}. Redis for hot data (${\sim}1$\,ms reads) and Huffman-compressed disk archives for cold data connected by a background LRU eviction daemon. Alongside with this, a \textbf{custom binary archive format} having a 24-byte header achieves 45-67\% compression on conversation based text, using zero native dependencies. All tunables are consolidated into a single configuration object with environment variable support, making the system deployable across different environments without code changes.
...
We explore the technical level implementation in this section. The importance of each layer of the caching mechanism and it's seven components are thus being described.
...
The real test of any caching system is whether it actually delivers in production, especially on modest hardware. Our entire stack runs on a single 8-vCPU Intel Cascade Lake cloud instance (2\,GHz, 32\,GB RAM, no GPU). The search assistant runs as three containerized replicas (2\,vCPU, 2\,GB RAM limit each) with 10 async workers per replica i.e 30 workers total. Redis 7.4 runs in its own container capped at 2\,GB, and an nginx load balancer sits in front. All measurements below were taken from this live system in production.
...
\caption{Per-query cost at 50k queries/month (log scale). Commercial providers charge per-query API fees. lixSearch amortizes a fixed \$96/mo server cost. With 89\% cache hit rate, effective cost drops to \$0.0002/query-1{,}000$\times$ cheaper than SearchGPT, 24$\times$ cheaper than Perplexity.}
\label{fig:cost}
\end{figure}
...
Before building the caching layer, we surveyed the landscape of existing tools. None of them solved our problem completely, but each influenced our design in specific ways.
...
Redis is widely used as an LLM response cache. Frameworks like Semantic Kernel~\cite{semantic_kernel2023} and Haystack~\cite{haystack2023} support Redis as a cache backend. However, these typically use Redis as a flat key-value store. Our system uses three separate Redis databases with distinct TTL profiles and data formats, and adds the hybrid hot/cold tier with disk overflow-a pattern not found in existing frameworks.

\textbf{Conversation compression and archival.}
MemGPT~\cite{memgpt2023} addresses the context window limitation by paging conversation history between a main context and an external storage tier, analogous to virtual memory. Our approach is similar in spirit: the hot Redis window serves as ``main memory'' and the Huffman-compressed disk archive as ``swap.'' However, MemGPT focuses on autonomous LLM-driven memory management, while our system uses deterministic LRU eviction with fixed window sizes.

\textbf{Data compression for chat.}
Standard approaches use gzip or lz4 for compressing stored conversations. Our use of canonical Huffman coding is motivated by the small payload sizes typical of conversation archives ($<$100\,KB), where dictionary-based compressors have proportionally higher overhead. The pure-Python implementation eliminates native build dependencies, simplifying deployment in containerized environments.
...
We started with a simple goal: build a search assistant that could compete with SearchGPT, Perplexity, and Google's AI search-without their per-query price tags-and do it on a single 8-vCPU server with no GPU. Along the way, we discovered that the real engineering challenge was not in searching the web or calling an LLM, but in managing the state that accumulates around multi-turn conversations at scale on resource-constrained hardware.

The caching architecture that grew out of those challenges now powers every conversation on our search based pipeline. Each of its three layers-session context, semantic query deduplication, and URL embedding. This idea came from from a specific production pain point. The result is a system that is simple enough to understand and also effective enough to sustain indefinitely long conversations in production on commodity CPU infrastructure:
