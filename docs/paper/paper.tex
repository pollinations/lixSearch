...
So we built our search assistant from scratch. The initial version was deliberately simple. Instead of paying per-query fees to a proprietary search API, we used automated headless browser agents to perform web searches directly-the same way a human would. The agents navigated search engines, extracted results, fetched full page content, and fed it to an open LLM for synthesis. There was no retrieval-augmented generation (RAG), no vector database, no caching layer. Just a search agent pool, a language model, and a pipeline connecting them.

It worked. The per-query cost dropped dramatically-from \$0.03--0.10 with SearchGPT to fractions of a cent with open models and free search. But as users grew and conversations got longer, three problems emerged that threatened to erode every cost advantage we had built:
...
\begin{itemize}
\item \textbf{Layer 1: Session Context Window.} A rolling window of the 20 most recent messages in Redis, with automatic overflow to Huffman-compressed disk archives.
\item \textbf{Layer 2: Semantic Query Cache.} Catches rephrasings by comparing query embeddings via cosine similarity ($\tau \geq 0.90$), returning cached LLM responses and skipping the entire pipeline. Created from watching the same query hit our search agents dozens of times in slightly different words.
\item \textbf{Layer 3: URL Embedding Cache.} A global, cross-session store of pre-computed embedding vectors with 24-hour TTL. Created  from profiling and discovering that a significant fraction of embedding computations were redundant across concurrent sessions.
\end{itemize}

The system also introduced a \textbf{two-tier hybrid storage model}: Redis for hot data (${\sim}1$\,ms reads) and Huffman-compressed disk archives for cold data, connected by a background LRU eviction daemon. A \textbf{custom binary archive format} with a 24-byte header achieves 45--67\% compression on conversation text using zero native dependencies. All tunables are consolidated into a single configuration object with environment variable support, making the system deployable across different environments without code changes.

In production, this architecture serves an 89.3\% cache hit rate with just 1.38\,MB of Redis memory, enabling conversations that persist indefinitely across sessions, server restarts, and horizontal scaling events.

The remainder of this paper follows the arc of this journey. Section~\ref{sec:architecture} presents the architecture. Section~\ref{sec:design} explains the key design decisions. Section~\ref{sec:implementation} details the implementation. Section~\ref{sec:evaluation} evaluates production performance. Section~\ref{sec:related} surveys related work. Section~\ref{sec:limitations} discusses limitations, and Section~\ref{sec:conclusion} concludes.
...
principle: each caching concern gets its own logical partition within a single Redis instance (DB~0 for semantic query cache, DB~1 for URL embeddings, DB~2 for session context), and a single coordinator process ties them together. Figure~\ref{fig:architecture} illustrates 
the complete data flow when a user message arrives.

\begin{figure*}[t]
\centering
\input{architecture_inline.tex}
...
When the application requests context and Redis is empty (e.g., after LRU eviction), the system transparently re-hydrates by loading the last $k$ messages from the disk archive back into Redis. If Redis is entirely unavailable, the system falls back to disk-only reads to ensure no downtime.
...
Each layer operates on a separate Redis logical database (i.e., \textbf{DB\,0, DB\,1, DB\,2}) rather than using key prefixes within a single database. This provides three operational advantages:
...
Not every design decision was obvious from the start. Several emerged from mistakes, production incidents, or debates that only resolved after we tried the wrong approach first. This section captures the five key forks in the road and why we went the way we did.
...
\item \textbf{Zero native dependencies.} The code is pure Python based, eliminating build-time dependencies on C libraries. This simplifies deployment across diverse environments from local development to containerized production without platform-specific build steps.
\end{enumerate}
...
Early in development, we used a simple truncation strategy: keep the last $k$ messages, throw away the rest. This bit us when users returned to a conversation after an hour and asked ``what was that article you found earlier?'' The context was gone. Our system instead \emph{overflows} old messages to disk. This preserves the full conversation history for:
...
To justify the choice of Huffman over standard compressors, Table~\ref{tab:compression_comparison} compares our codec against zlib (level~1 and level~6) and lz4 on the same production conversation archives.

\begin{table}[h]
\centering
\caption{Compression ratio comparison across codecs on production archives (lower is better)}
\label{tab:compression_comparison}
{\scriptsize
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Archive} & \textbf{Raw} & \textbf{Huffman} & \textbf{zlib-1} & \textbf{lz4} \\
\midrule
...
At small payload sizes ($<$5\,KB), Huffman is competitive with zlib level-1 and outperforms lz4. At larger sizes, zlib's dictionary-based approach dominates. This confirms our design rationale: Huffman is the right choice for the \emph{typical} conversation archive (1--10\,KB), while zlib would be preferable for very large archives. Since 90\% of our production archives fall in the 1--10\,KB range, Huffman provides the best trade-off of compression ratio, speed, and zero native dependencies.

The throughput trade-off is explicit: our pure-Python Huffman codec processes at ${\sim}$700\,KB/s (encode) and ${\sim}$800\,KB/s (decode), compared to ${\sim}$50\,MB/s for native zlib. However, at typical archive sizes ($<$10\,KB), the absolute time difference is negligible (${\sim}$14\,ms for Huffman vs.\ $<$1\,ms for zlib). The codec is invoked only during overflow (hot $\to$ cold) and re-hydration (cold $\to$ hot)---infrequent operations that are not on the critical path of query serving.
...
\subsubsection{What Happens Without Each Layer}

To justify the three-layer design, we reason about the system's behavior if each layer were removed:

\begin{itemize}
\item \textbf{Without Layer 1:} Every context retrieval falls through to disk. At ${\sim}$107\,ms per disk read (Table~\ref{tab:compression}, 133-turn archive) versus 0.1\,ms for Redis, latency for context retrieval increases by three orders of magnitude. Multi-turn conversations become noticeably slower.

\item \textbf{Without Layer 2:} Every query---including rephrasings---triggers a full pipeline execution. Based on the observed 15--20\% rephrasing rate, this would increase LLM API calls by ${\sim}$18\% and search agent invocations proportionally. On a CPU-bound server, this directly reduces the number of concurrent sessions the system can serve.

\item \textbf{Without Layer 3:} Each session independently computes embeddings for shared URLs. With 10--30\% URL overlap across sessions and ${\sim}$200\,ms per embedding, a 10-session concurrent workload would waste 200--600\,ms of redundant CPU time per shared URL. On an 8-vCPU server, this is non-trivial.
\end{itemize}

We acknowledge that a controlled ablation study---disabling each layer in production and measuring end-to-end throughput---would provide stronger evidence. We plan to conduct this in future work with synthetic load generation to avoid impacting live users.
...
We identify several limitations that contextualize our results:

\textbf{Single-deployment evaluation.} All measurements come from one production deployment on one hardware configuration. We have not evaluated the system under controlled synthetic benchmarks, varying hardware, or adversarial workloads. The 89.3\% hit rate, latency numbers, and compression ratios are representative of \emph{our} workload but may differ for systems with different query distributions, session lengths, or concurrency patterns.

\textbf{No formal ablation.} While Section~\ref{sec:evaluation} provides a reasoned analysis of each layer's contribution, we have not conducted a controlled ablation study (disabling layers and measuring end-to-end impact). Such a study would require synthetic load generation to avoid disrupting live users.

\textbf{Brute-force similarity search.} The semantic cache compares the incoming query embedding against all cached embeddings for a given URL via linear scan ($O(n)$ where $n \leq 50$). This is adequate for the current bounded list size but would not scale to thousands of cached entries per URL. A vector index (FAISS, HNSW) would be needed for larger deployments.

\textbf{Huffman throughput.} The pure-Python Huffman codec processes at ${\sim}$700--800\,KB/s, roughly 60$\times$ slower than native zlib. For our typical payload sizes ($<$10\,KB), the absolute difference is negligible. For systems with very large conversation archives ($>$1\,MB), a native codec or optional C extension would be necessary.

\textbf{No encryption at rest.} Conversation archives on disk are compressed but not encrypted. The per-session privacy isolation described in this paper operates at the Redis key level, not the cryptographic level. Deployments handling sensitive data should add filesystem-level or application-level encryption.

\textbf{Cost comparison scope.} Our cost comparison with commercial providers (Table~\ref{tab:cost}) compares API fees against amortized infrastructure costs. It does not account for engineering time to set up and maintain the self-hosted system, which commercial APIs eliminate. The comparison is most favorable for teams that already operate their own infrastructure.
...
We started with a simple goal: build a search assistant that could compete with SearchGPT (\$0.03--0.10/query), Perplexity (\$0.005--0.018/query), and Google's grounded Gemini (\$0.014--0.035/query)-without their per-query price tags-and do it on a single 8-vCPU server with no GPU. Along the way, we discovered that the real engineering challenge was not in searching the web or calling an LLM, but in managing the state that accumulates around multi-turn conversations at scale on resource-constrained hardware.

The caching architecture that grew out of those challenges now powers every conversation on our search pipeline. Each of its three layers---session context, semantic query deduplication, and URL embedding reuse---was born from a specific production pain point. The result is a system that is simple enough to understand in an afternoon and effective enough to sustain indefinitely long conversations in production on commodity CPU infrastructure:

\begin{itemize}
\item \textbf{89.3\% Redis-level cache hit rate} across all three databases over a 6-day measurement window, with total Redis memory usage of only 1.38\,MB.
...
The key insight is that forever-continuous conversations-sessions that can span hours, days, or weeks without losing context-are not a feature of the LLM itself, but of the infrastructure around it. By combining a bounded hot window with unbounded cold storage, aggressive semantic deduplication, and cross-session embedding reuse, we built a system where conversation length is no longer a constraint. Users can return to a session days later and pick up exactly where they left off, with full history available for semantic retrieval.

\textbf{Future work.} The journey is not over. Potential improvements include: (1)~replacing the pure-Python Huffman codec with an optional C extension for higher throughput on large archives; (2)~supporting pluggable vector databases (FAISS, Chroma) as alternatives to the brute-force cosine similarity search in the semantic cache; (3)~implementing adaptive TTL policies that learn optimal cache lifetimes from observed query patterns; and (4)~exploring cross-session knowledge transfer, where insights from one conversation can inform responses in another.
...
{\footnotesize \textbf{License.} This paper is licensed under \href{https://creativecommons.org/licenses/by-nc-nd/4.0/}{CC BY-NC-ND 4.0}. You may share this work with attribution, but you may not modify it or use it commercially. The accompanying software is separately licensed under MIT. \textcopyright~2026 Ayushman Bhattacharya.}
...
\bibitem{perplexity_pricing}
Perplexity, ``Sonar API Pricing,'' 2025. [Online]. Available: \url{https://docs.perplexity.ai/guides/pricing}

\bibitem{google_grounding_pricing}
Google, ``Gemini API Pricing - Grounding with Google Search,'' 2025. [Online]. Available: \url{https://ai.google.dev/pricing}
