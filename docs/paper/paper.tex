...
So we built a three-layer caching architecture that grew organically out of the problems we faced in production. Each layer was born from a specific pain point:
...
With the problems identified, the architecture took shape around a simple principle: each problem gets its own caching layer, each layer gets its own Redis database, and a single coordinator fa\c{c}ade ties them together. Figure~\ref{fig:architecture} illustrates the complete data flow when a user message arrives.
...
\item \textbf{Zero native dependencies.} The codec is pure Python (${\sim}160$ lines), eliminating build-time dependencies on C libraries. This simplifies deployment across diverse environments from local development to containerized production without platform-specific build steps.
\end{enumerate}
...
With the architecture settled, the implementation came together over several weeks of iterative development. The caching layer is ${\sim}800$ lines of Python across eight modules compact enough to audit in an afternoon, but covering a surprising amount of ground. We describe the key implementation details of each component.
...
The caching architecture that grew out of those challenges now powers every conversation on our search assistant. Each of its three layers-session context, semantic query deduplication, and URL embedding reuse-was born from a specific production pain point, not from a whiteboard architecture session. The result is a system that is simple enough to understand in an afternoon (${\sim}800$ lines of Python) but effective enough to sustain indefinitely long conversations in production on commodity CPU infrastructure:
