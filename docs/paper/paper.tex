...
\node[app, minimum width=1.8cm] (a0) at (0,0) {\texttt{"CAv1"}\\4\,B magic};
\node[app, minimum width=2.4cm, right=0pt of a0] (a1) {Created\\8\,B (f64)};
\node[app, minimum width=2.4cm, right=0pt of a1] (a2) {Modified\\8\,B (f64)};
\node[app, minimum width=1.8cm, right=0pt of a2] (a3) {Turns\\4\,B (u32)};

% Label
\node[lbl, above=0.15cm of a1.north east] {Application Header (24\,B fixed)};

% Huffman codec header (variable)
\node[codec, minimum width=1.8cm, below=0.3cm of a0.south west, anchor=north west] (h0) {\texttt{"HCv1"}\\4\,B magic};
\node[codec, minimum width=1.8cm, right=0pt of h0] (h1) {Data len\\4\,B (u32)};
\node[codec, minimum width=1.6cm, right=0pt of h1] (h2) {$n$ syms\\2\,B (u16)};
\node[codec, minimum width=2.2cm, right=0pt of h2] (h3) {Symbol table\\$2n$\,B pairs};

% Label
\node[lbl, left=0.1cm of h0] {Codec\\Header};

% Codec header continued + data
\node[codec, minimum width=1.8cm, below=0.3cm of h0.south west, anchor=north west] (h4) {Pad bits\\4\,B (u32)};
\node[data, minimum width=6.6cm, right=0pt of h4] (payload) {Compressed Bitstream (variable length)};

% Label
\node[lbl, left=0.1cm of h4] {Codec\\Data};

% Brace for Huffman payload
\draw[decorate, decoration={brace, amplitude=5pt, mirror}, thick, blue!40]
    ([xshift=-2pt, yshift=-2pt]h0.south west) -- ([xshift=-2pt, yshift=2pt]h0.north west |- a3.south west)
    node[midway, left=6pt, font=\tiny\itshape, text=blue!60, align=right] {Huffman\\payload\\(from byte 24)};

% Brace for app header
\draw[decorate, decoration={brace, amplitude=5pt}, thick, orange!50]
    ([xshift=-2pt, yshift=2pt]a0.north west) -- ([xshift=2pt, yshift=2pt]a3.north east)
    node[midway, above=6pt, font=\tiny\itshape, text=orange!70] {scannable without decompression};
