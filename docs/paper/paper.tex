...
Authentication is handled with a fallback strategy: the pool first attempts to connect with the configured password, and if authentication fails (e.g., on a local development server with no password set), falls back to unauthenticated access with a logged warning. This accommodates both production and development environments, though operators should ensure \texttt{REDIS\_PASSWORD} is always set in production to avoid silently connecting without auth.
...
This is the \emph{Redis-level} aggregate hit rate across all three databases, reflecting all key lookups including internal operations (TTL refreshes, list reads, existence checks). It is not a direct measure of query-level cache hits, but rather an indicator of how effectively the system keeps its working set in Redis. The high rate is driven primarily by Layer~1 (session context), where repeated context lookups within a session consistently find messages in Redis. Layer~2 (semantic cache) contributes when users rephrase queries within the 5-minute TTL window.
