...
\subsection{Cost Comparison: SearchGPT vs.\ Our System}
...
\textbf{Our cost breakdown.} Our system has \emph{zero per-query API fees}. LLM inference runs through an open model API at no per-call cost. Web search is performed by Playwright-based stealth browser agents that scrape results directly with no search API subscription. Embedding computation (sentence-transformers, all-MiniLM-L6-v2) runs locally on CPU via a dedicated embedding service. The \emph{only} recurring cost is the server itself: an 8-vCPU cloud instance at ${\sim}$\$96/month. Per-query cost is therefore purely amortized infrastructure.
