...
def load_or_create_planning_file(request_id: str) -> Dict[str, Any]:
    """Load existing planning file or create new one following deep_planning.py schema"""
    ...
    # Schema matches deep_planning.py structure
    return {
        "main_query": "",
        "max_tokens": 1500,
        "subqueries": [],
...
def add_subquery_result(planning_data: Dict[str, Any], subquery_id: int, result: str) -> Dict[str, Any]:
    """Add result for a subquery while maintaining schema"""
    # Find the subquery and add result
    for subquery in planning_data.get("subqueries", []):
        if subquery.get("id") == subquery_id:
            subquery["_result"] = result
            subquery["_completed_at"] = datetime.now(timezone.utc).isoformat()
            break
    
    # Update metadata
    if "_metadata" not in planning_data:
        planning_data["_metadata"] = {}
    
    if "_completed_subqueries" not in planning_data["_metadata"]:
        planning_data["_metadata"]["_completed_subqueries"] = []
    
    if subquery_id not in planning_data["_metadata"]["_completed_subqueries"]:
        planning_data["_metadata"]["_completed_subqueries"].append(subquery_id)
    
    # Save immediately after each result
    request_id = planning_data["_metadata"].get("request_id", "unknown")
...
async def resolve_subquery(subquery: Dict[str, Any], 
    ...
    if not (subquery.get("youtube") or subquery.get("document")):
        # Run web search for context if not direct response
        if not subquery.get("direct_response", False):
    ...
    if subquery.get("time_based_query"):
        from getTimeZone import get_local_time
        try:
            local_time = get_local_time(subquery.get("time_based_query"))
            messages.append({
                "role": "user",
                "content": f"Current time information: {local_time}\n\nNow provide the answer to: {query_text}"
            })
        except Exception as e:
    ...
    try:
        ...
        return f"[ERROR] Failed to resolve: {str(e)[:200]}"
...
async def run_deep_research_pipeline(
    ...
    1. Generates a plan with subqueries following deep_planning.py schema
    2. Resolves each subquery and saves results incrementally
    3. Yields results via SSE as they complete (if event_id provided)
    ...
    planning_data["_metadata"]["status"] = "planning"
    ...
    try:
        ...
        # Merge plan data into planning_data while preserving schema
        planning_data["main_query"] = plan_data.get("main_query", user_query)
        planning_data["max_tokens"] = plan_data.get("max_tokens", 1500)
        planning_data["subqueries"] = plan_data.get("subqueries", [])
        planning_data["_metadata"]["status"] = "executing"
        save_planning_file(request_id, planning_data)
        ...
        for idx, subquery in enumerate(subqueries, 1):
            ...
            try:
                ...
                planning_data = add_subquery_result(planning_data, subquery_id, result)
                
                # Yield the completed subquery via SSE if event_id provided
                if event_id:
                    yield_data = {
                    ...
                    yield format_sse("result", json.dumps(yield_data))
                
                logger.info(f"Subquery {subquery_id} completed")
                
            except Exception as e:
                logger.error(f"Error resolving subquery {subquery_id}: {e}")
                error_result = f"[ERROR] Failed to resolve: {str(e)[:200]}"
                planning_data = add_subquery_result(planning_data, subquery_id, error_result)
        ...
        planning_data["_metadata"]["status"] = "synthesizing"
        save_planning_file(request_id, planning_data)
        ...
        synthesis_messages = [
            {
                "role": "system",
                "content": "You are a research synthesis expert. Create a comprehensive, well-structured response that integrates all the research findings. Format with clear sections and use markdown."
            },
            {
                "role": "user",
                "content": f"Original Query: {user_query}\n\nResearch Findings:\n" + 
                          "\n\n".join([
                              ...
                              f"{sq.get('_result', '[No result]')}"
                              for sq in subqueries if sq.get('_result')
                          ]) +
        ...
        try:
            ...
            planning_data["_synthesis"] = synthesis_result
            planning_data["_metadata"]["status"] = "completed"
            planning_data["_metadata"]["completed_at"] = datetime.now(timezone.utc).isoformat()
            save_planning_file(request_id, planning_data)
            ...
            planning_data["_synthesis"] = f"[ERROR] Synthesis failed: {str(e)}"
            planning_data["_metadata"]["status"] = "completed_with_errors"
            save_planning_file(request_id, planning_data)
        ...
        # Format final response with TASK blocks
        ...
        planning_data["_metadata"]["status"] = "failed"
        planning_data["_metadata"]["error"] = str(e)
        request_id = planning_data.get("_metadata", {}).get("request_id", "unknown")
        save_planning_file(request_id, planning_data)
...
def format_planning_response(planning_data: Dict[str, Any]) -> str:
    ...
    # Main query
    main_query = planning_data.get("main_query", "Unknown")
    parts.append(f"<TASK>Query: {main_query}</TASK>\n\n")
    
    # Planning overview
    subqueries = planning_data.get("subqueries", [])
    max_tokens = planning_data.get("max_tokens", "N/A")
    parts.append(f"<TASK>Research Plan</TASK>\n")
    parts.append(f"Total Subqueries: {len(subqueries)}\n")
    parts.append(f"Max Tokens Budget: {max_tokens}\n\n")
    
    # Individual results
    if subqueries:
        parts.append("<TASK>Research Findings</TASK>\n\n")
        for subquery in subqueries:
            subquery_id = subquery.get("id")
            query = subquery.get("q", "N/A")
            result = subquery.get("_result", "[No result]")
            
            parts.append(f"<TASK>Subquery {subquery_id}: {query}</TASK>\n")
            parts.append(f"{result}\n\n")
    
    # Synthesis
    if planning_data.get("_synthesis"):
        parts.append("<TASK>Final Synthesis</TASK>\n")
        parts.append(f"{planning_data.get('_synthesis')}\n\n")
    
    # Metadata
    metadata = planning_data.get("_metadata", {})
    status = metadata.get("status", "unknown")
    parts.append(f"<TASK>Status: {status}</TASK>\n")
    if metadata.get("completed_at"):
        parts.append(f"Completed At: {metadata.get('completed_at')}")
