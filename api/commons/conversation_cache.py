...
class ConversationCacheManager:
    """
    Manages conversation history with semantic windowing, compression, and efficient lookup.
    Stores compressed conversation entries with embeddings for semantic similarity matching.
    """
    ...
    def __init__(self, 
        ...
        Args:
            window_size: Number of conversation turns in active window
            max_entries: Maximum cached entries per session
            ttl_seconds: Time-to-live for cache entries
            compression_method: 'zlib', 'gzip', or 'lz4'
        ...
        # Cache storage: deque for window management
        self.cache_window: deque = deque(maxlen=window_size)
        self.full_cache: Dict[str, dict] = {}  # Full cache with embeddings
        self.embeddings_cache: Dict[str, np.ndarray] = {}  # Embeddings lookup
        self.latest_response: Optional[str] = None
        
    def add_to_cache(self, query: str, response: str, metadata: Optional[Dict] = None) -> None:
        """
        Add a query-response pair to cache with compression and embeddings.
        
        Args:
            query: User query
            response: System response
            metadata: Optional metadata (sources, tool_calls, etc.)
        """
        if len(query) < 10:  # Skip very short queries
            return
    ...
    def query_cache(self, 
        ...
        """
        Query cache using semantic similarity.
        
        Args:
            query: Query string
            use_window: Search only in active window (faster)
            similarity_threshold: Override default threshold
            return_compressed: Return compressed response (False = decompress)
            
        Returns:
            Tuple of (best_match_entry, similarity_score)
        ...
        try:
            ...
            # Return result if threshold exceeded
            if best_match and best_score >= threshold:
                result_entry = dict(best_match)
                
                # Decompress response if requested
    ...
    def batch_query_cache(self, queries: List[str], use_window: bool = True, top_k: int = 3) -> List[Tuple[Optional[Dict], float]]:
        """
        Batch query multiple related queries and return top-k matches per query.
        
        Args:
            queries: List of query strings
            use_window: Search only in active window
    ...
    def get_window_context(self) -> str:
        """
        Get formatted cache window context for system prompt injection.
        
        Returns:
            Formatted window context string
        """
    ...
    def _compress(self, data: str) -> bytes:
        """Compress response using configured method."""
    ...
    def _decompress(self, data: bytes, method: str) -> str:
        """Decompress response using specified method."""
        try:
            if method == "gzip":
